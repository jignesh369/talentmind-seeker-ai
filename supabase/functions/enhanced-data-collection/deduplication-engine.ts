...
export class DeduplicationEngine {
  ...
  static deduplicate(candidates: CandidateProfile[]): DeduplicationResult {
    console.log(`🔄 Starting enhanced deduplication of ${candidates.length} candidates...`);
    ...
    for (const candidate of candidates) {
      ...
      if (duplicates.length > 0) {
        ...
        console.log(`🔗 Enhanced merge: ${duplicates.length + 1} profiles → ${mergedProfile.name}`);
      } else {
    ...
    console.log(`✅ Enhanced deduplication: ${result.originalCount} → ${result.deduplicatedCount} (-${result.duplicatesFound})`);
    return result;
  }
  ...
  private static calculateEmailSimilarity(email1: string, email2: string): number {
    ...
    if (domain1 === domain2) {
      return this.levenshteinDistance(user1, user2) <= 2 ? 0.8 : 0;
    }
